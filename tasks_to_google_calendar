        ...
        try {
          ...
          for (let i = 0; i < tasks.items.length; i++) {
            ...
            if (
              ...
              var event = CalendarApp.getCalendarsByName(`Tasks`)[0].createAllDayEvent(task.title,
                new Date(task.due),
              );
              ...
              Logger.log('event created' + 'Event ID: ' + event.getId() + new Date(task.due));
            }
            else {
              if ((new Date(task.due).getDate() < new Date().getDate())) {
                CalendarApp.getCalendarsByName(`Tasks`)[0].getEventsForDay(new Date(task.due))[eventsTitleList.indexOf(task.title)].setAllDayDates(new Date(task.due), new Date(new Date().setDate(new Date().getDate() + 1)))
              }
